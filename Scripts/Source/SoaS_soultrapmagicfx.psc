ScriptName SoaS_SoulTrapMagicFX extends ActiveMagicEffect
...
Event OnEffectFinish(Actor Target, Actor Caster)
	...
	if victim
		...
		if DeadAlready == False
			if playerref.TrapSoul(victim) == true
				trace(victim + " is, in fact, dead.  Play soul trap visFX")
				TrapSoundFX.play(playerref)       ; play TrapSoundFX sound from player
				TrapImod.apply()                                  ; apply isMod at full strength
				TargetVFX.Play(victim,4.7,playerref)              ; Play TargetVFX and aim them at the player
				CasterVFX.Play(playerref,5.9,victim)
				TargetFXS.Play(victim,2)    						; Play Effect Shaders
				CasterFXS.Play(playerref,3)
			else
